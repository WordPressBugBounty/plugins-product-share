...
<?php
    ...
    $psfw_layout_page_nonce = wp_create_nonce( 'psfw-layout-nonce' );

    // Checking Nonce [Generated by layout.php] on page load
    if( wp_verify_nonce( $psfw_layout_page_nonce, 'psfw-layout-nonce' ) ){

        //Get the active plugin page from the $_GET param
        $psfw_plugin_name = isset($_GET['page']) ? sanitize_key( wp_unslash( $_GET['page'] ) ) : '';
        
        //Get the active tab from the $_GET param
        $psfw_default_tab = null;
        $psfw_curTab = isset($_GET['tab']) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : $psfw_default_tab;
    }
...
<?php 
    $tab = "<a href='?page=product-share' class='nav-tab ".($psfw_curTab===null ? 'nav-tab-active' : null)."'> ".__('General', 'product-share')."</a>";
    $tab .= "<a href='?page=product-share&tab=advanced' class='nav-tab ".($psfw_curTab==='advanced' ? 'nav-tab-active' : null)."'> ".WPXtension_Setting_Fields::pro_not_exist(Product_Share::check_plugin_state('product-share-pro')).__(' Advanced', 'product-share')."</a>";
?>
  
  <?php echo wp_kses_post( apply_filters('psfw_admin_setting_tab', $tab, $psfw_curTab) ); ?>
</nav>
            ...
            <form method="post" action="options.php">
                ...
                <div class="tab-content">
                    ...
                    <?php 
                        ...
                        do_action('psfw_setting_tab_content', $psfw_plugin_name, $psfw_curTab); 
                ...
                <p class="submit submitbox psfw-setting-btn">
                    ...
                    $psfw_current_page = 'product-share';
                    $psfw_current_tab = $psfw_curTab;

                    $psfw_reset_url_args = array(
                        'action'   => 'reset',
                        '_wpnonce' => wp_create_nonce( 'psfw-settings' ),
                    );

                    $psfw_action_url_args = array(
                        'page'    => $psfw_current_page,
                        'tab'     => $psfw_current_tab,
                    );

                    $psfw_reset_url  = add_query_arg( wp_parse_args( $psfw_reset_url_args, $psfw_action_url_args ), admin_url( 'admin.php' ) );
                     ...
                     * @since 1.2.12
                     * 
                     * @return $psfw_curTab returns null for General tab
                     * 
                     */ 
                    ...
                    if( Product_Share::check_plugin_state('product-share-pro') ){
                        submit_button( __( 'Save Settings', 'product-share' ), 'primary', 'psfw-save-settings', false); 
                    ?>
                        <a onclick="return confirm('<?php esc_html_e( 'Are you sure to reset?', 'product-share' ) ?>')" class="submitdelete" href="<?php echo esc_url( $psfw_reset_url ) ?>"><?php esc_attr_e( 'Reset Current Tab', 'product-share' ); ?></a>
                    <?php
                    }
                    if( !Product_Share::check_plugin_state('product-share-pro') && null === $psfw_curTab ){
                        submit_button( __( 'Save Settings', 'product-share' ), 'primary', 'psfw-save-settings', false); 
                    ?>
                        <a onclick="return confirm('<?php esc_html_e( 'Are you sure to reset?', 'product-share' ) ?>')" class="submitdelete" href="<?php echo esc_url( $psfw_reset_url ) ?>"><?php esc_attr_e( 'Reset Current Tab', 'product-share' ); ?></a>
                    <?php
